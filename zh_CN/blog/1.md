...
上网搜一下，只能搜到x86（32位）的跟踪方法：`orig_eax = ptrace(PTRACE_PEEKUSER, childpid, 4 * ORIG_EAX, NULL);`。

x86和x86_64（64位）是两种CPU架构，因为x86和x86_64的CPU指令长度、寄存器类型不同，`orig_eax = ptrace(PTRACE_PEEKUSER, childpid, 4 * ORIG_EAX, NULL);`在x86_64环境下根本没用，甚至编译报错`ORIG_EAX`没定义。
...
很容易看到，前面是`#ifdef __x86_64__`，即判断当前是否为x86_64，而ORIG_EAX在else里定义，即当前为是否为x86架构——难怪会出现编译错误。
...
在x86中，EAX用来储存函数入口，在x86_64中是RAX。

把ORIG_EAX换成ORIG_RAX，运行……

一个`-1`出现在我们眼前。

为啥是-1？这个是x86和x86_64的指令长度不一样导致的。
...
// syscallbasecount为运行后输出\0（然后直接退出的那个程序的危险系统调用数量。
...
#include </dev/random>      // Linux，让编译器读取随机值

#include <CON>              // Windows，让编译器读取控制台

``` 

能让编译器不断读取字符设备，永远编译下去；`main[-1u]={1};`可以产生体积巨大达16GB的可执行文件；2016版的HUSTOJ可以从`#include "/var/www/JudgeOnline/include/db_info.inc.php"`的编译错误信息中读出管理员的帐号名和密码……
